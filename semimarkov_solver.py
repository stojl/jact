...
def construct_callback(
    prob_callback: Optional[Callable[..., jnp.ndarray]],
    cashflow_callback: Optional[Callable[..., jnp.ndarray]],
):
    @jax.jit
    def callback(p, p_point, dB_left, dB_right, step_size, t_left, t_right):
        prob_value = prob_callback(p, p_point) if callable(prob_callback) else None
        cashflow_value = (
            cashflow_callback(p, p_point, dB_left, dB_right, step_size, t_left, t_right)
            if callable(cashflow_callback)
            else None
        )

        return {"probability": prob_value, "cashflow": cashflow_value}
...
def heun_scheme_solver(
    ...
    if result["probability"] is not None:
        ...
        result["probability"] = jax.tree_util.tree_map(
            lambda arr: concatenate_init_probability(arr, init_prob_callback_value),
            result["probability"],
        )
