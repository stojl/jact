...
def compute_derivative(
    ...
    inflow_integral = jnp.sum(
        inflow_avg * jnp.expand_dims(p, axis=-3), axis=(-2, -1)
    )  # B X J
    inflow_point_integral = inflow_plus[..., 0, :-1] * jnp.expand_dims(p_point, axis=-2)
    inflow_point_integral = jnp.sum(inflow_point_integral, axis=-1)
