    ...
    for j in range(J):
        ...
        for i in range(J):
            ...
            if m_p is not None:
                ...
                term_p = jnp.sum(m_avg * p[:, i, :], axis=-1)
                inflow_terms_for_j.append(term_p)
                
                if i == 0:
                    term_p_point = jnp.sum(m_p_slice * p_point, axis=-1)
                    inflow_terms_for_j.append(term_p_point)
                    
        if inflow_terms_for_j:
            summed_term = reduce(jax.lax.add, inflow_terms_for_j)
            next_inflow.append(summed_term)
        else:
            next_inflow.append(jnp.zeros((B,)))
    next_inflow = jnp.stack(next_inflow, axis=1)
